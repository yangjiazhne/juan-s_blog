...
export default {
  ...
  methods: {
    ...
    async deleteConfirmHandle() {
      let { data } = await specialPartSectionApi.deleteSpecialPartSectionByUid(
        this.selectIds
      );
      if (data.code === 1) {
        this.$message({
    ...
    async submitForm() {
      ...
      if (!this.isUpdate) {
        ...
        // 如果是编辑
        data = await specialPartSectionApi.updateSpecialPartSectionByUid(
          this.form
        );
      }
    ...
    async getList() {
      let { data } = await specialPartSectionApi.querySpecialPartSectionPage(
        this.searchParam
      );
...
.special-part-section-container {

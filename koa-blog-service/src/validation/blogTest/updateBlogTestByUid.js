 ...
 * @param blogSummary 不能为空
 * @param isOriginal 不能为空
 */
module.exports = (
    uid, blogTitle, blogSummary, isOriginal
) => {
    /**
     * 保存接口校验参数获取
     *
    ...
    let _blogSummary = isEmpty(blogSummary) ? '' : blogSummary
    ...
    if (!_blogSummary) {
        return {
            errorMsg: 'blogSummary不能为空',
            isValid,
        }
    }
    if (!_isOriginal) {
        return {
            errorMsg: 'isOriginal不能为空',
            isValid,
        }
    }

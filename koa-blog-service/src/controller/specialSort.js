const specialSortService = require('../service/specialSort')
const {
...
const saveSpecialSort = async ctx => {
    /**
     * 这里把数据库表里的字段都声明出来
    ...
    if (!isValid) { // 校验不通过
         ...
         *
         */
        let result = await specialSortService.querySpecialSortBySpecialSortName(specialSortName)
        ...
        if (result) { // 已存在，不保存
            ...
            let flag = await specialSortService.saveSpecialSort(params)
            if (flag) {
                ctx.success()
            }
        }
 ...
 */
const deleteSpecialSortByUid = async ctx => {
    ...
    if (!isValid) { // 校验不通过
        ...
        let result = await specialSortService.deleteSpecialSortByUid(uids)
        if (result) {
            ctx.success()
        }
    }
...
const querySpecialSortPage = async ctx => {
    /**
     * 这里把数据库表里的字段都声明出来
     * 除了 uid create_time update_time
     *
    ...
    if (!isValid) { // 校验不通过
        ...
        let result = await specialSortService.querySpecialSortPage(params)
        let {total} = await specialSortService.queryAllCountSpecialSort(params)
        let newResult = {
            result,
            total,
            currentPage,
...
const updateSpecialSortByUid = async ctx => {
    /**
     * 这里把数据库表里的字段都声明出来
    ...
    if (!isValid) { // 校验不通过
        ...
        // 补充参数
        let {create_time} = await specialSortService.querySpecialSortByUid(uid)
        params.createTime = create_time

        // 根据uid修改这条记录
        let result = await specialSortService.updateSpecialSortByUid(params)
        if (result) {
            ctx.success()
        }
    }
...
const querySpecialSortAll = async ctx => {
    console.log(ctx.request.body, 'ctx.request.body')
    let result = await specialSortService.querySpecialSortAll()
    ctx.success(result)
}

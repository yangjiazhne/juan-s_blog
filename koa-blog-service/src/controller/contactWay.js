const contactWayService = require('../service/contactWay')
const {
...
const saveContactWay = async ctx => {
    /**
     * 这里把数据库表里的字段都声明出来
    ...
    if (!isValid) { // 校验不通过
         ...
         *
         */
        let result = await contactWayService.queryContactWayByContactWay(contactWay)
        ...
        if (result) { // 已存在，不保存
            ...
            let flag = await contactWayService.saveContactWay(params)
            if (flag) {
                ctx.success()
            }
        }
 ...
 */
const deleteContactWayByUid = async ctx => {
    ...
    if (!isValid) { // 校验不通过
        ...
        let result = await contactWayService.deleteContactWayByUid(uids)
        if (result) {
            ctx.success()
        }
    }
...
const queryContactWayPage = async ctx => {
    /**
     * 这里把数据库表里的字段都声明出来
     * 除了 uid create_time update_time
     *
    ...
    if (!isValid) { // 校验不通过
        ...
        let result = await contactWayService.queryContactWayPage(params)
        let {total} = await contactWayService.queryAllCountContactWay(params)
        let newResult = {
            result,
            total,
            currentPage,
...
const updateContactWayByUid = async ctx => {
    /**
     * 这里把数据库表里的字段都声明出来
    ...
    if (!isValid) { // 校验不通过
        ...
        // 补充参数
        let {create_time} = await contactWayService.queryContactWayByUid(uid)
        params.createTime = create_time

        // 根据uid修改这条记录
        let result = await contactWayService.updateContactWayByUid(params)
        if (result) {
            ctx.success()
        }
    }
...
const queryContactWayAll = async ctx => {
    console.log(ctx.request.body, 'ctx.request.body')
    ...
    let result = await contactWayService.queryContactWayAll(params)
    ctx.success(result)
}

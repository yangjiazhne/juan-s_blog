const blogLikeService = require('../service/blogLike')
const {
...
const saveBlogLike = async ctx => {
    ...
    if (!isValid) { // 校验不通过
        ...
        let flag = await blogLikeService.saveBlogLike(params)
        if (flag) {
            ctx.success()
        }
    }
 ...
 */
const deleteBlogLikeByUid = async ctx => {
    ...
    if (!isValid) { // 校验不通过
        ...
        let result = await blogLikeService.deleteBlogLikeByUid(uids)
        if (result) {
            ctx.success()
        }
    }
...
const queryBlogLikePage = async ctx => {
    /**
     * 这里把数据库表里的字段都声明出来
     * 除了 uid create_time update_time
     *
    ...
    if (!isValid) { // 校验不通过
        ...
        let result = await blogLikeService.queryBlogLikePage(params)
        let {total} = await blogLikeService.queryAllCountBlogLike(params)
        let newResult = {
            result,
            total,
            currentPage,
...
const queryBlogLikeAll2 = async ctx => {
    ...
    let result = await blogLikeService.queryBlogLikeAll2(params)
    ctx.success(result)
}



const updateBlogLikeByUid = async ctx => {
    /**
     * 这里把数据库表里的字段都声明出来
    ...
    if (!isValid) { // 校验不通过
        ...
        // 补充参数
        let {create_time} = await blogLikeService.queryBlogLikeByUid(uid)
        params.createTime = create_time

        // 根据uid修改这条记录
        let result = await blogLikeService.updateBlogLikeByUid(params)
        if (result) {
            ctx.success()
        }
    }
...
const queryBlogLikeAll = async ctx => {
    console.log(ctx.request.body, 'ctx.request.body')
    let result = await blogLikeService.queryBlogLikeAll()
    ctx.success(result)
}

const aboutMeService = require('../service/aboutMe')
const {
...
const saveAboutMe = async ctx => {
    ...
    if (!isValid) { // 校验不通过
         ...
         *
         */
        let result = await aboutMeService.queryAboutMeByAdminUserId(adminUserId)
        ...
        if (result) { // 已存在，不保存
            ...
            let flag = await aboutMeService.saveAboutMe(params)
            if (flag) {
                ctx.success()
            }
        }
...
const queryAboutMeByAdminUserId = async ctx => {
 ...
 */
const deleteAboutMeByUid = async ctx => {
    let uids = ctx.request.body
    console.log(uids, 'uids')
    const {errorMsg, isValid} = deleteAboutMeByUidValidator(ctx.request.body)

    if (!isValid) { // 校验不通过
        ctx.fail(resCode.get(APP.PARAMETER_INVALID), errorMsg)
    } else {
        let result = await aboutMeService.deleteAboutMeByUid(uids)
        if (result) {
            ctx.success()
        }
    }
...
const queryAboutMePage = async ctx => {
    /**
     * 这里把数据库表里的字段都声明出来
     * 除了 uid create_time update_time
     *
    ...
    if (!isValid) { // 校验不通过
        ...
        let result = await aboutMeService.queryAboutMePage(params)
        let {total} = await aboutMeService.queryAllCountAboutMe(params)
        let newResult = {
            result,
            total,
            currentPage,
...
const updateAboutMeByUid = async ctx => {
    /**
     * 这里把数据库表里的字段都声明出来
    ...
    if (!isValid) { // 校验不通过
        ...
        // 补充参数
        let {create_time} = await aboutMeService.queryAboutMeByUid(uid)
        params.createTime = create_time

        // 根据uid修改这条记录
        let result = await aboutMeService.updateAboutMeByUid(params)
        if (result) {
            ctx.success()
        }
    }
...
const queryAboutMeAll = async ctx => {
    console.log(ctx.request.body, 'ctx.request.body')
    let result = await aboutMeService.queryAboutMeAll()
    ctx.success(result)
}

const friendLinkService = require('../service/friendLink')
const {
...
const saveFriendLink = async ctx => {
    /**
     * 这里把数据库表里的字段都声明出来
    ...
    if (!isValid) { // 校验不通过
         ...
         *
         */
        let result = await friendLinkService.queryFriendLinkByLinkName(linkName)
        ...
        if (result) { // 已存在，不保存
            ...
            let flag = await friendLinkService.saveFriendLink(params)
            if (flag) {
                ctx.success()
            }
        }
 ...
 */
const deleteFriendLinkByUid = async ctx => {
    ...
    if (!isValid) { // 校验不通过
        ...
        let result = await friendLinkService.deleteFriendLinkByUid(uids)
        if (result) {
            ctx.success()
        }
    }
...
const queryFriendLinkPage = async ctx => {
    /**
     * 这里把数据库表里的字段都声明出来
     * 除了 uid create_time update_time
     *
    ...
    if (!isValid) { // 校验不通过
        ...
        let result = await friendLinkService.queryFriendLinkPage(params)
        let {total} = await friendLinkService.queryAllCountFriendLink(params)
        let newResult = {
            result,
            total,
            currentPage,
...
const updateFriendLinkByUid = async ctx => {
    /**
     * 这里把数据库表里的字段都声明出来
    ...
    if (!isValid) { // 校验不通过
        ...
        // 补充参数
        let {create_time} = await friendLinkService.queryFriendLinkByUid(uid)
        params.createTime = create_time

        // 根据uid修改这条记录
        let result = await friendLinkService.updateFriendLinkByUid(params)
        if (result) {
            ctx.success()
        }
    }
...
const queryFriendLinkAll = async ctx => {
    let {
        linkName, linkIntro, linkAddress, linkEmail,
        isPublish, orderNum
    } = ctx.request.body
    ...
    let result = await friendLinkService.queryFriendLinkAll(params)
    ctx.success(result)
}
